...
class MP3toM4AConverter {
    private let logger = OSLog(subsystem: "at.untitledapps.iConvert", category: "MP3toM4A")

    func convert(_ sourceURL: URL) -> Bool {
        os_log("Converting MP3 to M4A: %{public}@", log: logger, type: .info, sourceURL.path)

        // Create destination URL
        let filename = sourceURL.deletingPathExtension().lastPathComponent
        let newFilename = "\(filename).m4a"
