...
class HEICtoPNGConverter {
    private let logger = OSLog(subsystem: "at.untitledapps.iConvert", category: "HEICtoPNG")

    func convert(_ sourceURL: URL) -> Bool {
        os_log("Converting HEIC to PNG: %{public}@", log: logger, type: .info, sourceURL.path)
