...
class WAVtoFLACConverter {
    private let logger = OSLog(subsystem: "at.untitledapps.iConvert", category: "WAVtoFLAC")

    func convert(_ sourceURL: URL) -> Bool {
        os_log("Converting WAV to FLAC: %{public}@", log: logger, type: .info, sourceURL.path)

        // Create destination URL
        let filename = sourceURL.deletingPathExtension().lastPathComponent
        let newFilename = "\(filename).flac"
        ...
        // Set FLAC as output file type
        if #available(macOS 11.0, *) {
            // Use string-based identifier since UTType doesn't have a built-in flac constant
            exportSession.outputFileType = AVFileType("public.audio.flac")
        } else {
