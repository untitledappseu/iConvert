...
class WAVtoMP3Converter {
    private let logger = OSLog(subsystem: "at.untitledapps.iConvert", category: "WAVtoMP3")

    func convert(_ sourceURL: URL) -> Bool {
        os_log("Converting WAV to MP3: %{public}@", log: logger, type: .info, sourceURL.path)

        // Create destination URL
        let filename = sourceURL.deletingPathExtension().lastPathComponent
        let newFilename = "\(filename).mp3"
