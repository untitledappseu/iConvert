...
class MOVtoMP4Converter {
    private let logger = OSLog(subsystem: "at.untitledapps.iConvert", category: "MOVtoMP4Converter")
    private var progressTimer: Timer?

    func convert(sourceURL: URL) {
        os_log("Starting MOV to MP4 conversion for %{public}@", log: logger, type: .info, sourceURL.lastPathComponent)
        ...
        exportSession.exportAsynchronously {
            ...
            DispatchQueue.main.async {
                ...
                case .completed:
                    os_log("MOV to MP4 conversion completed successfully", log: self.logger, type: .info)
    ...
    private func showProgressNotification(progress: Float) {
        ...
        content.title = "Converting MOV to MP4"
        ...
        let request = UNNotificationRequest(identifier: "movtomp4.progress", content: content, trigger: nil)
    ...
    private func showCompletionNotification(success: Bool) {
        ...
        if success {
            content.title = "MOV to MP4 Conversion Complete"
            content.body = "Your video has been successfully converted to MP4 format."
        } else {
            content.title = "MOV to MP4 Conversion Failed"
            content.body = "There was an error converting your video. Please try again."
        }

        content.sound = UNNotificationSound.default

        let request = UNNotificationRequest(identifier: "movtomp4.completion", content: content, trigger: nil)
