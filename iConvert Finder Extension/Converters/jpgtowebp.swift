...
class JPGtoWebPConverter {
    private let logger = OSLog(subsystem: "at.untitledapps.iConvert", category: "JPGtoWebP")

    func convert(_ sourceURL: URL) -> Bool {
        os_log("Converting JPG to WebP: %{public}@", log: logger, type: .info, sourceURL.path)
        ...
        // Convert to WebP using a temporary JPG file
        ...
        do {
            ...
            // Use cwebp command line tool to convert JPG to WebP
            let process = Process()
